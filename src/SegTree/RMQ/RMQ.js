...
class segtree {
  constructor(sz) {
    let n = 1;
    while (n < sz) n *= 2;
    this.node = new Array(2*n - 1).fill(Infinity);
  ...
  _getmin(tl, tr, k, l, r) {
    if (r <= tl || tr <= l) {
      ...
      this.hist.push(this);
      let vl = this._getmin(tl, tr, 2*k + 1, l, Math.floor((l+r)/2));

      this.col[2*k + 2] = 'yellow';
      this.hist.push(this);
      let vr = this._getmin(tl, tr, 2*k + 2, Math.floor((l+r)/2), r);
  ...
  getmin(l, r) {
    ...
    this.hist.clear();
    this.hist.push(this);
    return this._getmin(l, r, 0, 0, this.n);
  };
